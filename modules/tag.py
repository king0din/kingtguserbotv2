    ...
    if args:
        ...
        if parts[0].isdigit():
            ...
            if len(parts) > 1:
                ...
                if q.entities:
                    full_msg = q.text or q.message
                    cmd_length = len(full_msg) - len(args)
                    number_length = len(parts[0]) + 1
                    text_start = cmd_length + number_length
                    message_entities = extract_entities_for_text(full_msg, text_start, q.entities)
            else:
                is_reply = True
        else:
            message_text = args
            ...
            if q.entities:
                full_msg = q.text or q.message
                cmd_length = len(full_msg) - len(args)
                message_entities = extract_entities_for_text(full_msg, cmd_length, q.entities)
    else:
        is_reply = True
    
    reply_msg = await q.get_reply_message()
    if reply_msg and (is_reply or not message_text):
        message_text = reply_msg.text or "📢"
...
async def tag_admins(q):
    ...
    reply_msg = await q.get_reply_message()
    if reply_msg and (is_reply or not message_text):
        message_text = reply_msg.text or "📢 Adminler!"
        message_entities = reply_msg.entities
        is_reply = True
