...
# Kullanım: .klon @kullanıcı veya .klon 123456789 veya yanıtla .klon
...
async def klon_help(event):
    ...
    await event.edit(
        "**🎭 Klon Plugin**\n\n"
        "**Kullanım:**\n"
        "• `.klon @kullanıcı` - Kullanıcı adıyla\n"
        "• `.klon 123456789` - ID ile\n"
        "• Mesajı yanıtla + `.klon`\n\n"
        "**Diğer:**\n"
        "• `.unclon` - Orijinal profile dön\n"
        "• `.saveme` - Profilini kaydet\n"
        "• `.kloninfo` - Kayıtlı profili göster\n"
        "• `.resetklon` - Verileri sıfırla"
    )
...
@register(outgoing=True, pattern=r"^\.unclon$")
...
@register(outgoing=True, pattern=r"^\.resetklon$")
...
@register(outgoing=True, pattern=r"^\.kloninfo$")
...
CMD_HELP.update({
    "klon":
    "`.klon` <yanıt/@kullanıcı/ID> - Profili klonla\n"
    "`.unclon` - Orijinal profile dön\n"
    "`.saveme` - Profilini kaydet\n"
    "`.kloninfo` - Kayıtlı profili göster\n"
    "`.resetklon` - Verileri sıfırla"
